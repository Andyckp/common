...
public class MarketDataAeronContinuouslyProducer implements StartableProcess {
    ...
    public MarketDataAeronContinuouslyProducer(Aeron aeron, AeronArchive aeronArchive) {
        this.aeron = aeron;
        this.aeronArchive = aeronArchive;
        this.thread = new Thread(this::write, MarketDataAeronOnDemandProducer.class.getSimpleName());
    }

    private void write() {
        aeronArchive.startRecording(CHANNEL_URI, STREAM_MARKET_DATA_CAPTURE, LOCAL);

        try (ExclusivePublication publication = aeron.addExclusivePublication(CHANNEL_URI, STREAM_MARKET_DATA_CAPTURE)) {
